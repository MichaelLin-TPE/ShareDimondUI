...
const sendEmail = async () => {
  ...
  try {
    ...
    error.value = ''
    useAlert.success('信件已發出,請在15分鐘內更改密碼,謝謝')
  } catch (e) {
    console.error(e)
...
<template>
  <div class="about">
    ...
    <div class="input_email">
      <input v-model="email" placeholder="請輸入驗證電子郵件地址" autocomplete="email" />
    </div>

    <p v-if="error" class="error">{{ error }}</p>

    <button @click="sendEmail" :disabled="loading">
      <span v-if="!loading">傳送驗證信件</span>
...
button:hover:not(:disabled) {
  ...
  box-shadow: 0 6px 20px rgba(108, 242, 255, 0.35);
}
